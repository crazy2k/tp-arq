...
/* Adrian: En principio si se falla en el predictor de saltos no se puede ejecutar en paralelo ya que hasta que no pasen los ciclos no se puede decodificar nuevas instrucciones
 */
        ...
        VOID process_instruction(INS ins) {
            ...
            if (INS_IsMemoryWrite(ins)) {
                ...
                /* Adrian: En principio la condicion es mas complicada, se puede ejecutar en paralelo si el registro no esta esperando ser escrito por una instruccion anterior, por ejemplo, supone que tuviste dos instrucciones de load anteriores la primera hace un load a eax y la segunda a ebx, la primera hizo un miss en l1 y l2 y va a demorar 100 ciclos, la segunda un miss a l2 y va a demorar 10 ciclos, la instruccion que tenemos que ejecutar ahora es mover ebx a ecx, esta recien se va a poder ejecuar despues de 9 ciclos cuando llegue ebx desde l2, si la siguiente instruccion es por ejemplo mover esi a edi, se puede ejecutar despues de 10 ciclos y si la que sigue es mover ebx a eax, en principio se podria ejecutar en ese ciclo pero los procesadores esperarian a que se termine de ejecuar el load que hizo miss en l1 y l2, si no me equivoco en nuestro caso 89 ciclos.
                 Por otra parte, si lo que queremos en simular un procesador con mas de way, lo que tenemos que ver es que la segunda instruccion no utilice ningun registro que la primera escriba, ambas pueden leer desde el mismo registro (obviamente dependiendo del numero de puertos de lectura que tengamos en el fichero de registros
                 */
